...
//
// ALL_TOOL_KEYS / LEGACY_TOOL_MAP are derived from public/tool-catalog.json —
// that file is the single source of truth for which apps exist. Adding an app
// there is enough; nothing here needs editing. public/admin.html (Access
// Manager) and public/index.html fetch the same catalog client-side.
const TOOL_CATALOG = require('../public/tool-catalog.json').apps || [];
const ALL_TOOL_KEYS = TOOL_CATALOG.map(app => app.key);
const LEGACY_TOOL_MAP = {};
for (const app of TOOL_CATALOG) {
  for (const legacyId of (app.legacyIds || [])) {
    LEGACY_TOOL_MAP[legacyId] = app.key;
  }
}
